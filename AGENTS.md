...
- Load `.agents/skills/qa-verification/SKILL.md` when a completed change needs browser verification, smoke verification, report-only QA, or fix-enabled verification.
...
- Prefix nx commands with the workspace's package manager (for example `pnpm nx build`, `pnpm nx run`, `pnpm exec nx show project`) - avoids using globally installed CLI
- You have access to the Nx MCP server and its tools, use them to help the user
- For Nx plugin best practices, check `node_modules/@nx/<plugin>/PLUGIN.md`. Not all plugins have this file - proceed without it if unavailable.
- NEVER guess CLI flags - always check docs or `--help` first when unsure

## Scaffolding & Generators

- For scaffolding tasks (creating apps, libs, project structure, setup), ALWAYS invoke the `nx-generate` skill FIRST before exploring or calling MCP tools

## When to use nx docs / workspace truth

- USE workspace truth first for: projects, targets, dependencies, project graph questions, and target discovery
- USE docs for: advanced config options, unfamiliar flags, migration guides, plugin configuration, and edge cases
- The `nx-generate` skill handles generator discovery internally - do not call docs only to look up generator syntax
