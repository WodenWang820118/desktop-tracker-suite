...
For browser-verifiable `ng-tracker` tasks, `proofshot` can be used after implementation and before final sign-off, typically through `qa-verification`, to generate screenshots, session video, and a local proof summary for human review.
...
- `apps/ng-tracker`: Angular frontend tracker application
- `apps/nest-backend`: NestJS backend service
- `apps/express-backend`: Express backend service
- `apps/spring-backend`: Spring Boot backend service
- `apps/react-tracker`: React frontend tracker
- `apps/vue-tracker`: Vue frontend tracker
- `apps/tauri-shell`: Tauri desktop shell

Use repo-specific reviewers and skills with that topology in mind.

## Stack Conventions

- For Angular, NestJS, and Electron/Tauri work, load the matching file under `.agents/references/stack-conventions/` after reading `AGENTS.md`.
...
- Electron guidance should reflect main-process security, contextBridge, and IPC best practices.
