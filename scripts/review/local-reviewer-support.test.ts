...
test('resolveLocalReviewerRepoRoot finds the sibling workspace', () => {
  ...
  const currentRepo = resolve(workspace, 'nx-electron');
...
test('buildCheckpointReviewContext includes an explicit changed files section before the diff', () => {
  const context = buildCheckpointReviewContext({
    changedFiles: ['apps/react-tracker/src/app/app.tsx'],
    diffText:
    ...
    sample: {
      ...
      fileCount: 1,
      kind: 'small-ts',
      repoName: 'nx-electron',
      repoRoot: 'C:/software-dev/nx-electron',
      subject: 'Update copy',
      totalChangedLines: 4,
    },
  });
