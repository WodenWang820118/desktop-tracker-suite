...
// Hybrid review keeps GPT triage and local reviewer routing together.
...
export function buildHybridReviewReport(input: {
  ...
  const summary =
    escalationReasons.length > 0
      ? `Hybrid review completed for ${input.heuristics.file_count} file(s). Escalation recommended: ${escalationReasons.join('; ')}.`
